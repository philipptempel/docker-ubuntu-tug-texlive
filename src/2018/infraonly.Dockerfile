...
ADD --chown=latex:latex 2018/texlive.profile /texlive/install/texlive.profile
RUN mkdir /src \
    && chown -R latex:latex /src

RUN cd /texlive/install/ \
    && wget --user=anonymous --password=ftp --no-parent --no-verbose ftp://tug.org/historic/systems/texlive/2018/tlnet-final/install-tl-unx.tar.gz \
    && tar -xvf install-tl-unx.tar.gz \
    && cd install-tl-*/ \
    && ./install-tl -profile=/texlive/install/texlive.profile -repository=ftp://tug.org/historic/systems/texlive/2018/tlnet-final/ \
    && cd /texlive \
